 ...
 */
package com.jani.taulukkolistat;
...
public class Kayttoliittyma extends javax.swing.JFrame {
    ...
    double keskiIka = 0;
    int ikalista[];
    int ikalistaIndeksi = 0;
    private void paivitaKeskiIka() {
        if (ikalistaIndeksi == 0) {
            keskiIka = ikalista[0];
        }
        else {
            double summa = 0;
            ...
            keskiIka = summa / (ikalistaIndeksi+1);
            System.out.println(keskiIka);
        }
    ...
    private void jbtnLisaaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtnLisaaActionPerformed
        ...
        int[] uusi_ikalista = new int[ikalista.length+1];
        int indeksi = 0;
        for (int i : ikalista) {
            uusi_ikalista[indeksi] = i;
            indeksi++;
        }
        uusi_ikalista[ikalistaIndeksi] = Integer.valueOf(jtxtIka.getText());
        ikalista = uusi_ikalista;
        paivitaKeskiIka();
        ikalistaIndeksi++;
